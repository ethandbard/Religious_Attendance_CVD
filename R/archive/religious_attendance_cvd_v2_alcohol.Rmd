...
xpt$ALQ_missing_values <- rowSums(is.na(xpt))
xpt %>% 
  filter(ALQ_missing_values == 8)

xpt %>% 
  mutate(ALQ = case_when(
    ...
    #ALQ_missing_values == 8 ~ "Not Asked",
    TRUE ~ as.character("Not Asked"))) %>% 
  dplyr::select(starts_with("ALQ")) %>% 
  filter(ALQ == "Not Asked", ALQ_missing_values < 8) %>% 
  arrange(-ALQ120Q)
```
...
data <- data %>% 
  mutate(alcohol_consumption = case_when(
   ...
   # ALQ_missing_values == 8 ~ "Not Asked"
    ...
    #ALQ101 == 9 | ALQ110 == 9 | ALQ120Q == 999 | ALQ130 == 999 | ALQ140U | 9999 ~ "Not Asked",
    TRUE ~ as.character("Not Asked")
  )) %>% 
...
data %>% 
  dplyr::select(ALQ101, ALQ110, ALQ120Q, ALQ130, ALQ140Q, alcohol_consumption) %>% 
  filter(alcohol_consumption == "Not Asked")
...
data.frame(
  Models = c("Model 1m", 
             "Model 2m", 
             "Model 3m"),
  ChiSq_significance = c(with(model1m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
...
```{r}
blr_multi_model_fit_stats(model1m, 
                          model2m, 
                          model3m)
```
...
```{r}
tbl_merge(tbls = list(model1m_tbl, 
                      model2m_tbl, 
                      model3m_tbl),
          tab_spanner = c("Model 1m", 
                          "Model 2m", 
                          "Model 3m"))
```
