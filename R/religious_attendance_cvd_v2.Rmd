...
data <- data %>% 
  mutate(tobacco_use = case_when(
    SMQ020 == 2 & SMQ040 = 3 ~ "Never",
    SMQ040 %in% c(1,2,3) ~ "Current",
...
#Social + health
model2 <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption +fasting_glucose + mobility_limit + high_blood_pressure, data = data_small2, family = "binomial")

#all variables
model3 <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + 
                  alcohol_consumption +
