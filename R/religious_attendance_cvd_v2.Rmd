...
```{r}
model1_tbl <- tbl_regression(model1, exponentiate = FALSE) %>% 
...
model2_tbl <- tbl_regression(model2, exponentiate = FALSE) %>% 
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
model3_tbl <- tbl_regression(model3, exponentiate = FALSE) %>% 
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
