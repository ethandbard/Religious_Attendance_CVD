...
data <- data %>% 
  mutate(tobacco_use = factor(tobacco_use, levels = c("Never", "Current", "Former", "Not asked")))
...
frequency_table <- as.data.frame(table(data$age, data$gender, data$ethnicity, data$education, data$marital_status,
                                       ...
                                       data$high_blood_pressure,data$fasting_glucose, data$tobacco_use,
...
model1f <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out,
               data = filter(data_small1, gender == "Female"), family = "binomial")
...
model1m <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out,
               data = filter(data_small1, gender == "Male"), family = "binomial")

#Social + health
model2f <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption +fasting_glucose + mobility_limit + high_blood_pressure,
               data = filter(data_small2, gender == "Female"), family = "binomial")

model2m <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption +fasting_glucose + mobility_limit + high_blood_pressure, 
               data = filter(data_small2, gender == "Male"), family = "binomial")


#all variables
model3f <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption + fasting_glucose + mobility_limit + high_blood_pressure + religious_attendance, 
                data = filter(data_small3, gender == "Female"), family = "binomial")

model3m <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption + fasting_glucose + mobility_limit + high_blood_pressure + religious_attendance, 
                data = filter(data_small3, gender == "Male"), family = "binomial")
```
...
#10 fold cross validation to determine best classification threshold
folds <- createFolds(train.data1f$cvd, k = 10)
model <- train(as.factor(cvd) ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out,
      data = train.data1f,
      method = "glm",
      metric = "Precision",
...
predicted.classes1f <- ifelse(probabilities1f > 0.09, "pos", "neg")
...
df <- data.frame(predictions = predicted.classes1f,
           probabilities = probabilities1f,
           observed = test.data1f$cvd)
...
plot(roc(cvd ~ prob, data = test.data1f), main = "Model 1")
roc(cvd ~ prob, data = test.data1f)
...
predicted.classes1m <- ifelse(probabilities1m > 0.5, "pos", "neg")
...
df <- data.frame(predictions = predicted.classes1m,
           probabilities = probabilities1m,
           observed = test.data1m$cvd)
...
plot(roc(cvd ~ prob, data = test.data1m), main = "Model 1")
roc(cvd ~ prob, data = test.data1m)
...
predicted.classes2f <- ifelse(probabilities2f > 0.5, "pos", "neg")
...
data.frame(predictions = predicted.classes2f,
           probabilities = probabilities2f,
           observed = test.data2f$cvd) %>% 
  ggplot(aes(x = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke") ), y = probabilities, fill = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke")))) + 
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(title = "Model 2f: Estimated Probabilities vs Observed Cardiovascular Disease History", x = "", y = "Estimated probability") +
  scale_fill_grey(start = 0.6, end = 0.4)
...
predicted.classes2m <- ifelse(probabilities2m > 0.5, "pos", "neg")
...
data.frame(predictions = predicted.classes2m,
           probabilities = probabilities2m,
           observed = test.data2m$cvd) %>% 
  ggplot(aes(x = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke") ), y = probabilities, fill = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke")))) + 
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(title = "Model 2m: Estimated Probabilities vs Observed Cardiovascular Disease History", x = "", y = "Estimated probability") +
  scale_fill_grey(start = 0.6, end = 0.4)
...
predicted.classes3f <- ifelse(probabilities3f > 0.5, "pos", "neg")
...
data.frame(predictions = predicted.classes3f,
           probabilities = probabilities3f,
           observed = test.data3f$cvd) %>% 
  ggplot(aes(x = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke") ), y = probabilities, fill = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke")))) + 
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(title = "Model 3f: Estimated Probabilities vs Observed Cardiovascular Disease History", x = "", y = "Estimated probability") +
  scale_fill_grey(start = 0.6, end = 0.4)
...
data.frame(predictions = predicted.classes3m,
           probabilities = probabilities3m,
           observed = test.data3m$cvd) %>% 
  ggplot(aes(x = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke") ), y = probabilities, fill = factor(observed, labels = c("Never had a heart attack or stroke", "Had a heart attack or stroke")))) + 
  geom_boxplot() +
  theme_minimal() +
  theme(legend.position = "none") + 
  labs(title = "Model 3m: Estimated Probabilities vs Observed Cardiovascular Disease History", x = "", y = "Estimated probability") +
  scale_fill_grey(start = 0.6, end = 0.4)
