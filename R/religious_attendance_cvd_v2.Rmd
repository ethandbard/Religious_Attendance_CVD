...
#risk_mobility
data <- merge(data, xpt[c("SEQN", "PFQ049", "PFQ061B", "PFQ061C")], by = "SEQN", all.x = TRUE) %>% 
  dplyr::rename(mobility_limit = PFQ049,
                difficulty_walking = PFQ061B,
                difficulty_stairs = PFQ061C) %>% 
  dplyr::mutate(risk_mobility = case_when(
                mobility_limit == "Yes" ~ 1,
                TRUE ~ as.double(0)),
                difficulty_walking = factor(difficulty_walking, levels = c(1,2,3,4), labels = c("No difficulty", "Some difficulty", "Much difficulty", "Unable to do")),
                difficulty_stairs = factor(difficulty_stairs, levels = c(1,2,3,4), labels = c("No difficulty", "Some difficulty", "Much difficulty", "Unable to do")))
...
#Social + health
model2f <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption +fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure,
               data = filter(data_small2, gender == "Female"), family = "binomial")

model2m <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption +fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure, 
               data = filter(data_small2, gender == "Male"), family = "binomial")


#all variables
model3f <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption + fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure + religious_attendance, 
                data = filter(data_small3, gender == "Female"), family = "binomial")

model3m <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + alcohol_consumption + fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure + religious_attendance, 
...
data_small1f %>% 
  ...
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 1f Age Distribution")

data_small1m %>% 
  ggplot(aes(x = ethnicity, y = age)) + 
  geom_boxplot(fill = "gray") + 
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 1m Age Distribution")

data_small2f %>% 
  ggplot(aes(x = ethnicity, y = age)) + 
  geom_boxplot(fill = "gray") + 
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 2f Age Distribution")

data_small2m %>% 
  ggplot(aes(x = ethnicity, y = age)) + 
  geom_boxplot(fill = "gray") + 
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 2m Age Distribution")

data_small3f %>% 
  ggplot(aes(x = ethnicity, y = age)) + 
  geom_boxplot(fill = "gray") + 
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 3f Age Distribution")

data_small3m %>% 
  ggplot(aes(x = ethnicity, y = age)) + 
  geom_boxplot(fill = "gray") + 
  theme_minimal() + 
  labs(x = "", y = "Age", title = "Model 3m Age Distribution")
...
roc_obj <- roc(cvd ~ prob, data = test.data1m)
roc_obj
possible_thresholds <- coords(roc_obj)
best_threshold <- possible_thresholds %>% 
  mutate(balanced_accuracy = (specificity + sensitivity) / 2) %>% 
...
model2f_train <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + 
                  alcohol_consumption +
                  fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure, 
...
roc_obj <- roc(cvd ~ prob, data = test.data2f)
roc_obj
possible_thresholds <- coords(roc_obj)
best_threshold <- possible_thresholds %>% 
  mutate(balanced_accuracy = (specificity + sensitivity) / 2) %>% 
...
roc_obj <- roc(cvd ~ prob, data = test.data2m)
roc_obj
possible_thresholds <- coords(roc_obj)
best_threshold <- possible_thresholds %>% 
  mutate(balanced_accuracy = (specificity + sensitivity) / 2) %>% 
...
model3f_train <- glm(cvd ~ age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use + 
                  alcohol_consumption +
                  fasting_glucose + difficulty_walking + difficulty_stairs + high_blood_pressure + religious_attendance, 
...
roc_obj <- roc(cvd ~ prob, data = test.data3f)
roc_obj
possible_thresholds <- coords(roc_obj)
best_threshold <- possible_thresholds %>% 
  mutate(balanced_accuracy = (specificity + sensitivity) / 2) %>% 
...
model4f <- glm(cvd ~ age + ethnicity + education  + food_ran_out + fasting_glucose + difficulty_walking + difficulty_stairs,
               data = data_small4f, family = "binomial")

model4m <- glm(cvd ~ age + ethnicity + education + food_ran_out + fasting_glucose + difficulty_walking + difficulty_stairs,
