...
data <- data %>% 
  mutate(alcohol_consumption = case_when(
    ...
    is.na(ALQ101) & is.na(ALQ101) ~ "Not Asked",
    is.na(ALQ120Q) ~ "Not Asked",
    is.na(ALQ130) ~ "Not Asked",
    ALQ101 == 9 | ALQ110 == 9 | ALQ120Q == 999 | ALQ130 == 999 | ALQ140U | 9999 ~ "Don't Know",
...
```{r, eval = FALSE}
frequency_table <- as.data.frame(table(data$gender, data$ethnicity, data$education, data$marital_status,
                    ...
                    rename(Gender = Var1,
                           ...
                           Education = Var3, 
                           Marital_status = Var4,
                           Poverty_level_category = Var5,
                           Health_insurance = Var6,
                           Number_healthcare_visits = Var7,
                           Received_food_stamps = Var8,
                           Worried_house_run_out_of_food = Var9,
                           Food_ran_out = Var10,
                           Mobility_limit = Var11,
                           High_blood_pressure = Var12,
                           Fasting_glucose = Var13,
                           Tobacco_use_5_days = Var14,
                           Alcohol_consumption = Var15,
                           Religious_attendance = Var16,
                           Heart_attack = Var17,
                           Stroke = Var18)
...
#all variables
model1 <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use_5_days + 
                  alcohol_consumption +
                  fasting_glucose + mobility_limit + high_blood_pressure + religious_attendance, 
                data = data_small1, family = "binomial")
...
model3 <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out,data = data_small3, family = "binomial")
```
...
```{r, eval=FALSE}
summary(step(model1, trace = 0, direction = 'both'))
summary(step(model2, trace = 0, direction = 'both'))
summary(step(model3, trace = 0, direction = 'both'))
```
...
x <- as.matrix(data_small1)

model1_lasso <- glmnet(x, y = as.factor(cvd), alpha = 1, family = "binomial")
```
...
```{r}
model1_train <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out + tobacco_use_5_days + 
                  alcohol_consumption +
                  fasting_glucose + mobility_limit + high_blood_pressure + religious_attendance, 
...
```{r}
model3_train <- glm(cvd ~ gender + age + rfm + ethnicity + education + marital_status + poverty_level_category + health_insurance + num_healthcare_visits + worried_house_run_out_food + food_ran_out, 
...
dimnames(conf.matrix)[[2]] <- c("neg", "pos")
#can run this if threshold is changed. need two prediction classes to calculate specificity and sensitivity
confusionMatrix(conf.matrix)
```
