...
data <- data %>%
  mutate(
    DPQ010 = replace(DPQ010, DPQ010 == 9, NA),
...
data <- data %>%
  group_by(SEQN) %>%
  mutate(depression_count = sum(c(is.na(c(
...
data <- merge(data, xpt[c("SEQN", "HOQ065")], by = "SEQN", all.x = TRUE) %>%
  rename(home = HOQ065)
data <- data %>%
  mutate(home = case_when(home == 1 ~ "Yes", 
                          home %in% c(2,3) ~ "No",
                          TRUE ~ as.character(home)),
    home = factor(home, levels = c("Yes", "No"), labels = c("Yes", "No")
  ))
...
data <- merge(data, xpt[c("SEQN", "HUQ050", "HUQ030")], by = "SEQN", all.x = TRUE) %>%
  rename(
    num_healthcare_visits = HUQ050,
    health_care_access = HUQ030
  ) %>%
  mutate(
    num_healthcare_visits = case_when(
      num_healthcare_visits %in% c(0, 1) ~ "1",
      TRUE ~ as.character(num_healthcare_visits)
    ),
    health_care_access = case_when(
      health_care_access %in% c(1,3) ~ "Yes",
      health_care_access %in% c(2) ~ "No",
      TRUE ~ as.character(health_care_access)
    ),
    health_care_access = factor(health_care_access,
...
data <- merge(data, xpt[c("SEQN", "FSQ165", "FSQ171", "FSD670ZC", "FSD032A", "FSD032B")],
  ...
  rename(
    ever_received_food_stamps = FSQ165,
...
data <- merge(data, xpt[c("SEQN", "BPQ020", "BPQ080")], by = "SEQN", all.x = TRUE) %>%
  rename(
    high_blood_pressure = BPQ020,
    cholesterol = BPQ080
  )

data <- data %>%
  mutate(cholesterol = factor(cholesterol, levels = c(2, 1), labels = c("No", "Yes")))
...
data <- merge(data, xpt[c("SEQN", "SSD044", "SSQ011")], by = "SEQN", all.x = TRUE) %>%
  rename(
    religious_attendance = SSD044,
    emotional_support = SSQ011
  )

data <- data %>%
  mutate(emotional_support = case_when(emotional_support == 1 ~ "Yes", emotional_support %in% c(2, 3) ~ "No", TRUE ~ as.character(emotional_support)),
         emotional_support = factor(emotional_support, levels = c("Yes", "No"), labels = c("Yes", "No")))
...
data <- data %>%
  mutate(
    congestive = factor(congestive, levels = c(1, 2), labels = c("Yes", "No")),
...
data <- data %>%
  mutate(diabetes = factor(diabetes, levels = c(2, 1, 3), labels = c("No", "Yes", "Borderline")))
...
data$marital_status <- factor(data$marital_status,
  levels = c("Never Married", "Married", "Previously Married"),
...
data$poverty_level_category <- factor(data$poverty_level_category, c(3, 2, 1), 
                                      c("Poverty Index > 1.85",
                                      "Poverty Index in (1.30, 1.85)", 
                                        "Poverty Index <= 1.3"))
...
data$health_insurance <- factor(data$health_insurance, c("1", "2"), c("Covered", "Not Covered"))
...
data <- data %>%
  mutate(
    religious_attendance_num = religious_attendance,
...
```{r}
sumtable(data, digits = 3)
```

```{r, eval = FALSE}
# output variable frequency table to .csv
sumtable(data, digits = 3,  out = "csv", file = "../out/summary_table.csv")

# output full dataset to .csv
source_data %>%
  write.csv(file = "../out/source_data.csv")
```
...
# output variable frequency table to .csv
sumtable(data_f, digits = 3, out = "csv", file = "../out/summary_table_f.csv")
sumtable(data_m, digits = 3, out = "csv", file = "../out/summary_table_m.csv")
...
shapiro.test(data$depression)
ad.test(data$depression)
```


# Univariate Analysis / Multicollinearity Analysis
...
# only when they are normally distributed
# these variables are not

# Center and Scale continuous variables
# data$age <- scale(data$age)
# data$age <- center(data$age)
...
blr_vif_tol(model_3m)
```

## Control + religious attendance
...
```{r}
summary(model_4f)
blr_model_fit_stats(model_4f)
blr_vif_tol(model_4f)
```
...
```{r}
summary(model_4m)
blr_model_fit_stats(model_4m)
blr_vif_tol(model_4m)
```
...
model_4f_tbl <- tbl_regression(model_4f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
model_4m_tbl <- tbl_regression(model_4m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
Nagelkerke <- c(blr_rsq_nagelkerke(control_f),
                 blr_rsq_nagelkerke(model_1f),
                 blr_rsq_nagelkerke(model_2f),
                 blr_rsq_nagelkerke(model_3f),
                 blr_rsq_nagelkerke(model_4f),
                 blr_rsq_nagelkerke(model_f))

McFadden <- c(blr_rsq_mcfadden_adj(control_f),
                 blr_rsq_mcfadden_adj(model_1f),
                 blr_rsq_mcfadden_adj(model_2f),
                 blr_rsq_mcfadden_adj(model_3f),
                 blr_rsq_mcfadden_adj(model_4f),
                 blr_rsq_mcfadden_adj(model_f))
...
Nagelkerke <- c(blr_rsq_nagelkerke(control_m),
                 blr_rsq_nagelkerke(model_1m),
                 blr_rsq_nagelkerke(model_2m),
                 blr_rsq_nagelkerke(model_3m),
                 blr_rsq_nagelkerke(model_4m),
                 blr_rsq_nagelkerke(model_m))

McFadden <- c(blr_rsq_mcfadden_adj(control_m),
                 blr_rsq_mcfadden_adj(model_1m),
                 blr_rsq_mcfadden_adj(model_2m),
                 blr_rsq_mcfadden_adj(model_3m),
                 blr_rsq_mcfadden_adj(model_4m),
                 blr_rsq_mcfadden_adj(model_m))
...
# Re-organize model variable sets

```{r}
# Control + demo
demo_f <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status, data = data_f, family = "binomial")

demo_m <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status, data = data_m, family = "binomial")


# control + demo + risk
demo_risk_f <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI, data = data_f, family = "binomial")

demo_risk_m <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI, data = data_m, family = "binomial")


# alternative order: control + demo + sdoh
demo_sdoh_f <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression, data = data_f, family = "binomial")

demo_sdoh_m <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression, data = data_m, family = "binomial")


# control + demo + risk + sdoh
demo_risk_sdoh_f <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression, data = data_f, family = "binomial")

demo_risk_sdoh_m <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression, data = data_m, family = "binomial")


# all (include religious attendance)
full_f <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression
                   + religious_attendance, data = data_f, family = "binomial")

full_m <- glm(cvd ~ age + num_healthcare_visits + ethnicity + education + marital_status
                   + high_blood_pressure + cholesterol + diabetes + cigarette_use + BMI
                   + food_ran_out + health_insurance + home + poverty_level_category + 
                        emotional_support + health_care_access + depression
                   + religious_attendance, data = data_m, family = "binomial")
```
...
model_names <-  c("Demographics",
                     ...
                     "Demographics, Risk Factors, SDOH",
                     "Full Model")

Nagelkerke <- c(blr_rsq_nagelkerke(demo_f),
                 blr_rsq_nagelkerke(demo_risk_f),
                 blr_rsq_nagelkerke(demo_sdoh_f), 
                 blr_rsq_nagelkerke(demo_risk_sdoh_f),
                 blr_rsq_nagelkerke(full_f))

McFadden <- c(blr_rsq_mcfadden_adj(demo_f),
                 blr_rsq_mcfadden_adj(demo_risk_f),
                 blr_rsq_mcfadden_adj(demo_sdoh_f),
                 blr_rsq_mcfadden_adj(demo_risk_sdoh_f),
                 blr_rsq_mcfadden_adj(full_f))

AIC <- c(AIC(demo_f),
                 AIC(demo_risk_f),
                 AIC(demo_sdoh_f),
                 AIC(demo_risk_sdoh_f),
                 AIC(full_f))

BIC <- c(BIC(demo_f),
                 BIC(demo_risk_f),
                 BIC(demo_sdoh_f),
                 BIC(demo_risk_sdoh_f),
                 BIC(full_f))

#with(model, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))

#with(full_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))

Chisq <- c(with(demo_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_risk_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_sdoh_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_risk_sdoh_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(full_f, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)))
...
Nagelkerke <- c(blr_rsq_nagelkerke(demo_m),
                 blr_rsq_nagelkerke(demo_risk_m),
                 blr_rsq_nagelkerke(demo_sdoh_m), 
                 blr_rsq_nagelkerke(demo_risk_sdoh_m),
                 blr_rsq_nagelkerke(full_m))

McFadden <- c(blr_rsq_mcfadden_adj(demo_m),
                 blr_rsq_mcfadden_adj(demo_risk_m),
                 blr_rsq_mcfadden_adj(demo_sdoh_m),
                 blr_rsq_mcfadden_adj(demo_risk_sdoh_m),
                 blr_rsq_mcfadden_adj(full_m))

AIC <- c(AIC(demo_m),
                 AIC(demo_risk_m),
                 AIC(demo_sdoh_m),
                 AIC(demo_risk_sdoh_m),
                 AIC(full_m))

BIC <- c(BIC(demo_m),
                 BIC(demo_risk_m),
                 BIC(demo_sdoh_m),
                 BIC(demo_risk_sdoh_m),
                 BIC(full_m))

#with(model, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))

#with(full_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))

Chisq <- c(with(demo_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_risk_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_sdoh_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(demo_risk_sdoh_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)),
           with(full_m, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)))
...
```{r}
# make OR tables
demo_f_tbl <- tbl_regression(demo_f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_risk_f_tbl <- tbl_regression(demo_risk_f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_sdoh_f_tbl <- tbl_regression(demo_sdoh_f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_risk_sdoh_f_tbl <- tbl_regression(demo_risk_sdoh_f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
full_f_tbl <- tbl_regression(full_f, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
tbl_merge(
  tbls = list(
    ...
    demo_risk_f_tbl,
    demo_sdoh_f_tbl,
    demo_risk_sdoh_f_tbl,
    full_f_tbl
  ),
  tab_spanner = c("Demographics",
                     "Demographics and Risk Factors",
                     "Demographics and SDOH",
                     "Demographics, Risk Factors, SDOH",
                     "Full Model")
...
```{r}
# make OR tables
demo_m_tbl <- tbl_regression(demo_m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_risk_m_tbl <- tbl_regression(demo_risk_m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_sdoh_m_tbl <- tbl_regression(demo_sdoh_m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
demo_risk_sdoh_m_tbl <- tbl_regression(demo_risk_sdoh_m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
...
full_m_tbl <- tbl_regression(full_m, exponentiate = TRUE) %>%
  modify_table_styling(
    columns = c(estimate, ci),
    rows = reference_row %in% TRUE,
    missing_symbol = "Ref."
