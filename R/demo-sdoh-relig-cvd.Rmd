...
data <- merge(data, xpt[c("SEQN","SMQ680")], by = "SEQN", all.x = TRUE) %>% 
  rename(tobacco_5_days = SMQ680)

data <- data %>% 
  mutate(tobacco_5_days = factor(tobacco_5_days, levels = c(1,2), labels = c("Yes", "No")))
...
data <- data %>% 
  group_by(SEQN) %>% 
  mutate(depression_count = sum(c(is.na(c(DPQ010, DPQ020, DPQ030, DPQ040, DPQ050, DPQ060, DPQ070, DPQ080, DPQ090)))))
...
data <- data %>% 
  mutate(depression = case_when(
    depression_count <= 5 ~ sum(DPQ010, DPQ020, DPQ030, DPQ040, DPQ050, DPQ060, DPQ070, DPQ080, DPQ090, na.rm = TRUE)))
...
data <- merge(data, xpt[c("SEQN","HOQ065")], by = "SEQN", all.x = TRUE) %>% 
  rename(home = HOQ065)
data <- data %>% 
  mutate(home = factor(home, levels = c(1,2,3,7,9, '.'), labels = c("Owned or being bought", "Rented", "Other Arrangement", "Refused", "Don't know", "Missing")))
...
data <- merge(data, xpt[c("SEQN", "HUQ050", "HUQ030")], by = "SEQN", all.x = TRUE) %>% 
  ...
  mutate(num_healthcare_visits = case_when(
    ...
    health_care_access = factor(health_care_access, levels = c(1,2,3,7,9,"."), labels = c("Yes", "There is no place", "There is more than one place", "Refused", "Don't know", "Missing"))
  )
...
```{r}
control <- glm(cvd ~ age + gender + num_healthcare_visits, data = data, family = "binomial")
summary(control)
blr_model_fit_stats(control)
```
...
```{r}
alcohol_consumption <- glm(cvd ~ alcohol_consumption, data = data, family = "binomial")
summary(alcohol_consumption)
blr_model_fit_stats(alcohol_consumption)
chisq.test(data$cvd, data$alcohol_consumption)
```
...
```{r}
tobacco_5_days <- glm(cvd ~ alcohol_consumption, data = data, family = "binomial")
summary(tobacco_5_days)
blr_model_fit_stats(tobacco_5_days)
chisq.test(data$cvd, data$tobacco_5_days)
```
