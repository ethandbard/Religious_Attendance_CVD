...
```{r, warning=FALSE}
chi_square_matrix <- matrix(nrow = ncol(data), ncol = ncol(data))
for (i in 1:ncol(data)) {
  for (j in 1:ncol(data)) {
    if (i != j) {
      # Perform a chi-square test for independence
      chi_square_test <- chisq.test(data[, i], data[, j])
      # Store the results of the chi-square test in the matrix
      chi_square_matrix[i, j] <- chi_square_test$p.value
    }
  }
}

# Print the chi-square matrix
rownames(chi_square_matrix) <- colnames(data)
colnames(chi_square_matrix) <- colnames(data)
data.frame(chi_square_matrix)
data.frame(chi_square_matrix) %>% 
  write.csv(file = "../out/chi_square_matrix.csv")
```


# Kruskal-Wallis matrix

```{r}
kruskal_matrix <- matrix(nrow = ncol(data), ncol = ncol(data))
for (i in 1:ncol(data)) {
  for (j in 1:ncol(data)) {
    if (i != j) {
      # Perform a kruskal test for independence
      kruskal_test <- kruskal.test(data[, i] ~ data[, j], data = data)
      # Store the results of the chi-square test in the matrix
      kruskal_matrix[i, j] <- kruskal_test$p.value
    }
  }
}

rownames(kruskal_matrix) <- colnames(data)
colnames(kruskal_matrix) <- colnames(data)
data.frame(kruskal_matrix)
data.frame(kruskal_matrix) %>% 
  write.csv(file="../out/kruskal_wallis_matrix.csv")
```
...
```{r}
model_4f <- glm(cvd ~ age+ 
         num_healthcare_visits+ 
         religious_attendance,
         data = data_f,
         family = "binomial")

model_4m <- glm(cvd ~ age+ 
         num_healthcare_visits+ 
         religious_attendance,
         data = data_m,
         family = "binomial")
